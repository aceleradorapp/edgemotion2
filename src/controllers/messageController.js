...
module.exports = {
    async create(req, res) {
        try {
            const senderId = req.user.id;  // Usuário autenticado
            const { content, recipientIds } = req.body;

            if (!content || !Array.isArray(recipientIds) || recipientIds.length === 0) {
                return res.status(400).json({ error: 'Content e recipientIds são obrigatórios.' });
            }
